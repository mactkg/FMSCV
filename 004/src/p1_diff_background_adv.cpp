...
int main() {
    ...
    // capture, resize, grayscale(background)
    capture >> background;
    ...
    while (1){
        // capture, resize, grayscale(realtime)
        capture >> target;
        ...
        // binarize the diff image
        cv::threshold(diff, mask, thresh, 255, cv::THRESH_BINARY);
        smallTarget.copyTo(result, mask); // target image ==(mask)==> result canvas
